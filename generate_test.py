...
def transparentOverlay(path):
    ...
    for i in range(0, 10):
        ...
        readimg = cv.resize(readimg, (218, 218))
...
def generateImage():
    ...
    with open("./data/dataset/test.txt", "w") as wf:
        for i in range(0, 20):
            ...
            cv.imwrite("./data/dataset/test/" + str(i) + ".jpg", overlay)
...
#         # cv.waitKey(0)
#         # cv.destroyAllWindows()
#         cv.imwrite("./garbage/valid/images/" + str(i) + ".jpg", overlay)
#         with open("./garbage/valid/labels/" + str(i) + ".txt", "w") as wf:
#             wf.write(label)
#             wf.flush()
